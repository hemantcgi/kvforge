...
def _write_pending_update(uc_id: str, new_chunks: int) -> None:
    """Record the count of newly synced chunks so the Studio banner can read it."""
    import json, time
    path = ROOT / "examples" / uc_id / "pending_update.json"
    try:
        existing = 0
        if path.exists():
            try:
                existing = json.loads(path.read_text()).get("chunks", 0)
            except Exception:
                pass
        path.write_text(json.dumps({"chunks": existing + new_chunks, "ts": int(time.time())}))
    except OSError:
        pass
    ...
    async def run(self, connector_id: str, uc_id: str, trigger: str) -> str:
        ...
        try:
            ...
            if new_chunk_count > 0:
                _write_pending_update(uc_id, new_chunk_count)
                asyncio.create_task(
