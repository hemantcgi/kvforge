...
async def _run_test(connector_type: str, creds: dict) -> dict:
    if connector_type == "gdrive":
        ...
        lang = creds.get("language", "en").strip().lower() or "en"
        ...
        if r.status_code == 200:
            return {"ok": True, "detail": f"Wikipedia ({lang}) reachable — '{topic}' found"}
        if r.status_code == 404:
            return {"ok": False, "error": f"Article '{topic}' not found on {lang}.wikipedia.org — check the title spelling"}
