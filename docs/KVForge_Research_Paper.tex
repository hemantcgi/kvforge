...
We evaluate KVForge across four heterogeneous corpora---customer support dialogue, biomedical literature (PubMedQA), machine reading comprehension (SQuAD 2.0), and technical documentation (Amazon Bedrock User Guide)---on a single AWS g5.xlarge instance. KVForge reaches Phase 3 (parametric answering active) on all four corpora, with PRS of 0.755--0.863. Phase 2 delivers \textbf{2.7$\times$ generation speedup} over standard text-in-context RAG; Phase 3 delivers \textbf{3.6$\times$}. A new factual evaluation using exact-match, token-F1, and an LLM judge on held-out questions shows that the 3B model struggles across all four corpora, with exact-match near zero and modest token-F1 and judge scores. KV mean-pool and full-token injection do not consistently improve over text RAG in this initial real run, and the parametric mode is mixed. The PRS cosine-based accuracy proxy correlates only weakly with the factual metrics (Pearson \textit{r} = -0.12--0.43), and the cosine component reaches the Phase 3 threshold in all four corpora while the factual metrics do not. These results establish that training signal quality and a reliable factual gate, not just latency, are the dominant bottlenecks for deploying parametric answering in small language models.
...
\begin{itemize}
  ...
  \item \textbf{Phase 2 --- KV Injection:} Pre-computed mean-pooled KV tensors are injected directly into the model's \texttt{past\textbackslash\{\}\_key\textbackslash\{\}\_values} cache at query time, skipping the forward pass over the retrieved context. Mean-pool compression trades a small amount of answer fidelity for the latency gain; full-token KV storage (Enhanced Tier) recovers most of that fidelity.
...
Per-layer tensors \texttt{[1, n\textbackslash\{\}\_kv\textbackslash\{\}\_heads, seq\textbackslash\{\}\_len, head\textbackslash\{\}\_dim]} are mean-pooled over the sequence dimension, stacked into shape \texttt{[L, 2, H, d\textbackslash\{\}\_h]}, serialized to float16, and stored as base64 in the Qdrant payload field \texttt{kv\textbackslash\{\}\_cache}.
...
At query time, the inference module: (1) embeds the query and retrieves top-K chunks; (2) compares each chunk's \texttt{kv\textbackslash\{\}\_version} against \texttt{current\textbackslash\{\}\_lora\textbackslash\{\}\_version}; (3) if \textbf{all} chunks are fresh, injects their KV tensors via \texttt{past\textbackslash\{\}\_key\textbackslash\{\}\_values}; (4) if \textbf{any} chunk is stale, falls back to Phase 1 text-in-context for the entire query and enqueues stale chunks for background recomputation.
...
Before each LoRA training round, the \texttt{sleep\textbackslash\{\}\_faq\textbackslash\{\}\_generator} queries a cloud LLM (Gemini 2.5 Flash, Claude, or GPT-4.1) to generate N diverse question-answer pairs per chunk. This is sleep-time compute \cite{ref19}: offline work that improves future inference quality without impacting query latency.
...
\textbf{Figure 4} shows the tier weight system. The SQLite replay buffer (\texttt{core/replay\textbackslash\{\}\_buffer.py}) stores \texttt{(chunk\textbackslash\{\}\_id, text, tier)} rows derived from generated FAQs. Table~\ref{tab:replay-buffer} lists the tier conditions and replay weights.
...
\begin{itemize}
  \item \textbf{Targets:} \texttt{q\textbackslash\{\}\_proj}, \texttt{k\textbackslash\{\}\_proj}, \texttt{v\textbackslash\{\}\_proj} attention projections
...
After each evaluation round, FAQ questions with \texttt{accuracy\textbackslash\{\}\_ratio \$\textbackslash\{\}geq\$ 0.85} are embedded and stored in \texttt{known\textbackslash\{\}\_good\textbackslash\{\}\_queries} in \texttt{version.json}, seeding the Phase 3 confidence gate.
...
\begin{figure}[htbp]
  ...
  \caption{Figure 10: Per-chunk KV storage by format and model size. TurboQuant makes full per-token storage tractable for high-CIS chunks (\textasciitilde{}15 MB vs 67 MB float16 for Llama-3.1-8B). Mean-pool (Active Tier, V1) remains the default at 115--131 KB/chunk.}
  \label{fig:10}
\end{figure}
...
Groups are packed into uint8 (two 4-bit values per byte). Decompression: \texttt{values = q \$\textbackslash\{\}times\$ scale + zero}. For d\_h=128 and 4-bit quantization: 64 bytes vs. 256 bytes float16 --- 4$\times$ compression.
...
All backends implement Python \texttt{@runtime\textbackslash\{\}\_checkable} Protocol classes (structural typing, no inheritance). Available backends are listed in Table~\ref{tab:backends}:
...
Each use-case runs on a dedicated GPU (UC1: GPU 0, UC2: GPU 1, UC3: GPU 2, UC4: GPU 3) via \texttt{CUDA\textbackslash\{\}\_VISIBLE\textbackslash\{\}\_DEVICES}.
...
\textbf{Figure 11} shows query-response latency by phase, measured on UC4 with Llama-3.2-3B-Instruct via vLLM, median of 50 queries. The latency gap between phases is driven by how much of the transformer stack is exercised at query time.
...
In Phase 1, the retrieved context (five 600-token chunks) is concatenated with the prompt and passed through the full model before generation begins. The retrieval component itself is negligible (\textasciitilde{}12 ms), so the 1,852 ms latency is almost entirely attributable to encoding 3,000 tokens of context. Phase 2 eliminates this re-encoding step by injecting pre-computed KV tensors for the retrieved chunks directly into \texttt{past\textbackslash\{\}\_key\textbackslash\{\}\_values}. The generation pass now only needs to process the query tokens and the cached context, reducing latency to 692 ms --- a 2.7$\times$ speedup. Phase 3 removes retrieval entirely for high-confidence queries, so the model generates from parameters alone, reaching 510 ms (3.6$\times$ faster than Phase 1). These gains are especially valuable for high-throughput deployments where context re-encoding is the dominant GPU cost, and they show that the initial indexing investment is quickly amortized at query time.
...
\begin{figure}[htbp]
  ...
  \caption{Figure 15: PRS cosine accuracy ratio vs. factual correctness (0.5$\cdot$token-F1 + 0.5$\cdot$LLM-judge) per question. The cosine proxy is a weak predictor of factual correctness; Pearson r ranges from -0.12 (UC2) to 0.43 (UC1/UC4).}
  \label{fig:15}
\end{figure}
...
\textbf{Table 9:} Full E4 ablation (\texttt{n=50} per mode) on UC2 PubMedQA and UC3 SQuAD. Because the replay buffer is empty, tier-weighted and uniform replay draw identical replay batches. The LLM-judge score is 0.0 for all conditions. The UC3 \texttt{uniform\textbackslash\{\}\_heuristic} variant was trained on only one FAQ.


\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig17_ablation.png}
  \caption{Figure 17: E4 ablation (\texttt{n=50}) on UC2 PubMedQA and UC3 SQuAD. PRS is similar across the cloud-LLM variants because the empty replay buffer makes tier-weighted and uniform replay identical; token-F1 is much lower on SQuAD. The UC3 heuristic variant is an under-trained baseline (one FAQ).}
  \label{fig:17}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Figure 18: Per-layer attention divergence between true prefill and KV-injected attention distributions. Mean KL is 0.82--1.08 for full-token and 0.85--1.14 for mean-pool; the gap is corpus-dependent, not consistently in favor of full-token injection.}
  \label{fig:18}
\end{figure}
...
Our experiments across four heterogeneous corpora demonstrate that all four reach Phase 3 (PRS 0.755--0.863) on a single AWS g5.xlarge instance, with Phase 2 delivering \textbf{2.7$\times$ generation speedup} and Phase 3 delivering \textbf{3.6$\times$}. However, the new real factual evaluation shows that the 3B model is far less capable on held-out questions than the initial dry-run simulation suggested: exact-match is zero across all four modes, token-F1 and LLM-judge correctness are modest, and neither KV mean-pool nor full-token injection consistently improves over text RAG. The PRS cosine-based accuracy proxy also correlates weakly with the factual metrics (Pearson \textit{r} = -0.12--0.43) and reaches the Phase 3 threshold in every corpus while the factual metrics do not. The primary finding is therefore that training signal quality and a reliable factual gate---not just latency---are the critical bottlenecks for deploying parametric answering in small language models.
