#!/usr/bin/env bash
# start_dashboard.sh — Start the KVForge dashboard for Use Case 4: Amazon Bedrock User Guide
#
# Loads the pre-trained LoRA checkpoint and KV tensors for this corpus
# and starts the monitoring dashboard on port 8084.
#
# Usage (from repo root):
#   bash examples/usecase4_bedrock_userguide/start_dashboard.sh
set -euo pipefail
...
if [ ! -d "$UC_DIR/lora_checkpoints" ]; then
  ...
  echo "Run the full pipeline first: bash examples/usecase4_bedrock_userguide/run_pipeline.sh"
fi

PORT=$(python -c "import json; print(json.load(open('$CONFIG')).get('dashboard_port', 8084))")

echo "Starting KVForge dashboard for Use Case 4: Amazon Bedrock User Guide"
echo "  Config  : $CONFIG"
echo "  Port    : $PORT"
