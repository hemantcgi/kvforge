...
|----------|-------------|
| `decide_inference_mode(chunks, current_lora_version, phase=2, kds_threshold=None)` | Returns `"kv_injection"` or `"text_fallback"`. KV injection requires `phase >= 2`, all chunks fresh and cached, and every chunk's `kds` >= `kds_threshold`. `kds_threshold=None` fails closed to text fallback. |
